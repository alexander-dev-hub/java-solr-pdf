 ...
 * Detect Bold, Italic and Strike Through text using PDFBox with VB.NET </a>
 * <br/>
 * <a href="http://www.filedropper.com/exampledocument"> Example Document.pdf
 * </a>
 * <p>
 * This {@link PDFTextStripper} subclass shows how to extract text with the
 * styles used in the example document.
 * </p>
 * <p>
 * BEWARE: This merely is a proof of concept! In particular the tests in
 * {@link TransformedRectangle#underlines(TextPosition)} and
 * {@link TransformedRectangle#strikesThrough(TextPosition)} only work with the
 * restriction described in the methods themselves.
 * </p>
 * <a href="https://github.com/mkl-public/testarea-pdfbox1/issues/1">
 * PDFStyledTextStripper StrikeThrough Bug #1 </a> <br/>
 * <a href="https://www.dropbox.com/s/5chty5f4yotkb7s/style-test.pdf?dl=1">
 * style-test.pdf </a>
 * <p>
 * Indeed, the 's' of 'stroked' was not recognized as style strikethrough. This
 * appears due to a border case where float / double arithmetics give you a
 * headache. To make this work, the tolerance of the
 * {@link TransformedRectangle} method <code>strikesThrough</code> (and also
 * <code>underlines</code>) has been changed.
 * </p>
 * 
...
public class PDFStyledTextStripper extends PDFTextStripper {
	 ...
	 * @return
	 */
	String determineStyle(TextPosition textPosition) {
		 String result = "";
	...
	static class TransformedRectangle {
		...
		boolean strikesThrough(TextPosition textPosition) {
			try {
				...
				// TODO: This is a very simplistic implementation only working for horizontal
				// text without page rotation
				// and horizontal rectangular strikeThroughs with p0 at the left bottom and p2
				// at the right top

				// Check if rectangle horizontally matches (at least) the text
				if (p0.getX() > matrix.getXPosition() + textPosition.getWidth() * .1f
						|| p2.getX() < matrix.getXPosition() + textPosition.getWidth() * .9f)
					return false;
				// Check whether rectangle vertically is at the right height to underline
				double vertDiff = p0.getY() - matrix.getYPosition();
				if (vertDiff < 0 || vertDiff > textPosition.getFont().getFontDescriptor().getAscent()
						* textPosition.getFontSizeInPt() / 1000.0)
					return false;
		...
		boolean underlines(TextPosition textPosition) {
			try {
				...
				// TODO: This is a very simplistic implementation only working for horizontal
				// text without page rotation
				// and horizontal rectangular underlines with p0 at the left bottom and p2 at
				// the right top
				...
				// Check whether rectangle vertically is at the right height to underline
				double vertDiff = p0.getY() - matrix.getYPosition();
				if (vertDiff > 0 || vertDiff < textPosition.getFont().getFontDescriptor().getDescent()
						* textPosition.getFontSizeInPt() / 500.0)
					return false;
				// Check whether rectangle is small enough to be a line
				return Math.abs(p2.getY() - p0.getY()) < 2;
